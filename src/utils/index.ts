...
export const getCalendarDates: (
  startDate: Date
) => { startDate: Date; endDate: Date } = startDate => {
  const endDate: Date = addDays(startDate, daysRange - 1);

  return { startDate, endDate };
};

export const formatFullDate: (date: Date | string) => string = date => {
  // type checking added because `date` stays as a String after fetching from localstorage
...
export const getDateLabels: (startDate: Date) => string[] = startDate => {
  ...
  for (let i = 0; i < daysRange; i++) {
    ...
    const dayLabel = `${formatOneDigitDate(day.getDate())}.${formatOneDigitDate(
      day.getMonth() + 1
    )}`;
